...
class Camera
    ...
    /**
     * Return the min exposure time value supported by the camera to dart.
     */
    public long getMinExposureTime() { return cameraFeatures.getExposureTime().getMinExposureTime();}

    /**
     * Return the max exposure time supported by the camera to dart.
     */
    public long getMaxExposureTime() {return cameraFeatures.getExposureTime().getMaxExposureTime();}

    /**
     * Sets a new exposure time from dart. From dart the exposire Time comes as a long, like 1 or -1
     *
     * @param result flutter result.
     * @param exposureTime new time in nanoseconds.
     */
    public void setExposureTime(@NonNull final Result result, long exposureTime) {
        final ExposureTimeFeature exposureTimeFeature = cameraFeatures.getExposureTime();
        exposureTimeFeature.setValue(exposureTime);
        exposureTimeFeature.updateBuilder(previewRequestBuilder);

        refreshPreviewCaptureSession(
                () -> result.success(exposureTimeFeature.getValue()),
                (code, message) ->
    ...
    /** Gets all the available lens apertures available by the camera forthe device to dart **/
    public float[] getAvailableLensApertures(){
        return cameraFeatures.getLensAperture().getAvailableLensApertures();
    }

    /** Return the current lens aperture of the camera to dart. */
    public float getLensAperture(){
        return cameraFeatures.getLensAperture().getValue();
    }

    /**
     * Sets a new lens aperture from dart. From dart the aperture comes as a float, like 1.3f or -1.3f
     *
     * @param result flutter result.
     * @param aperture new value.
     */
    public void setLensAperture(@NonNull final Result result, float aperture) {
        final LensApertureFeature lensApertureFeature = cameraFeatures.getLensAperture();
        lensApertureFeature.setValue(aperture);
        lensApertureFeature.updateBuilder(previewRequestBuilder);

        refreshPreviewCaptureSession(
                () -> result.success(lensApertureFeature.getValue()),
                (code, message) ->
                        result.error("setLensApertureFailed", "Could not set lens aperture.", null));
    }
    ...
    /**
     * Return the min iso value supported by the camera to dart.
     */
    public int getMinSensorSensitivity() { return cameraFeatures.getSensorSensitivity().getMinSensorSensitivity();}

    /**
     * Return the min iso value supported by the camera to dart.
     */
    public int getMaxSensorSensitivity() {return cameraFeatures.getSensorSensitivity().getMaxSensorSensitivity();}

    /**
     * Sets a new sensor sensitivity from dart. From dart the sensor sensitivity comes as a int, like 1 or -1
     *
     * @param result flutter result.
     * @param iso new value.
     */
    public void setSensorSensitivity(@NonNull final Result result, int iso) {
        final SensorSensitivityFeature sensorSensitivityFeature = cameraFeatures.getSensorSensitivity();
        sensorSensitivityFeature.setValue(iso);
        sensorSensitivityFeature.updateBuilder(previewRequestBuilder);

        refreshPreviewCaptureSession(
                () -> result.success(sensorSensitivityFeature.getValue()),
                (code, message) ->
                        result.error("setSensorSensivivitityFailed", "Could not set sensor sensitivity.", null));
    }
